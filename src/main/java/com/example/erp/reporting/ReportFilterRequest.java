...
public record ReportFilterRequest(LocalDate from, LocalDate to, UUID customerId, UUID categoryId, UUID accountId,
    ...
    public ReportFilterRequest {
        if (currencyCode != null && currencyCode.isBlank()) currencyCode = null;
        if (size == 0) size = 20;
    ...
    public void validate(DateBasis basis, java.util.Set<String> allowedSorts) {
        ...
        if (from.plusMonths(ReportPolicy.MAX_DATE_RANGE_MONTHS).isBefore(to)) throw new IllegalArgumentException("Report date range is too large");
        if (size > ReportPolicy.MAX_PAGE_SIZE) throw new IllegalArgumentException("Report page size is too large");
