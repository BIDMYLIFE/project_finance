...
class ProductsPageControllerTest {
    @Test
    void productsRouteReturnsOnlyProductsViewAndEmbeddedMessages() throws Exception {
        ...
        MockMvc mvc = MockMvcBuilders.standaloneSetup(new ProductsPageController(messages)).build();

        var result = mvc.perform(get("/products").locale(Locale.ENGLISH))
                .andExpect(status().isOk()).andExpect(view().name("products/list")).andReturn();

        assertThat(result.getModelAndView().getModel().get("messages")).asInstanceOf(org.assertj.core.api.InstanceOfAssertFactories.MAP)
                .containsEntry("products.heading", "translated:products.heading");
    }
}